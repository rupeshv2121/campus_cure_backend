...
export const register = async (req, res) => {
    try {
        ...
        if (role === "STUDENT") {
            await withRetry(() => prisma.studentProfile.create({
                data: {
                    userId: user.id,
            ...
            await withRetry(() => prisma.facultyProfile.create({
                data: {
                    userId: user.id,
                    department: "Not Set",
                    branch: "Not Set",
            ...
            await withRetry(() => prisma.adminProfile.create({
                data: {
                    userId: user.id,
                    adminLevel: role === "SUPER_ADMIN" ? AdminLevel.SUPER : AdminLevel.NORMAL,
                    assignedDepartments: [],
                    allowedCategories: [],
                },
            }));
        ...
        if (error instanceof Error && error.message.includes("ETIMEDOUT")) {
            res.status(503).json({
                error: "Database connection timeout. The database might be unavailable or sleeping. Please try again in a moment.",
            });
            return;
        }
...
export const login = async (req, res) => {
    try {
        ...
        if (user.role === Role.ADMIN || user.role === Role.SUPER_ADMIN) {
            const adminProfile = await withRetry(() => prisma.adminProfile.findUnique({
                where: { userId: user.id },
            }));
            if (adminProfile) {
                await withRetry(() => prisma.adminProfile.update({
                    where: { userId: user.id },
