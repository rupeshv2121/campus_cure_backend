...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        const complaintsByTypeMap = new Map();
        const complaintsByDeptMap = new Map();
        complaints.forEach((complaint) => {
            ...
            complaintsByTypeMap.set(complaint.category, (complaintsByTypeMap.get(complaint.category) || 0) + 1);
        ...
        const complaintsByType = Array.from(complaintsByTypeMap.entries()).map(([name, value]) => ({
            name,
            value,
        }));
